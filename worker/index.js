...
//         esta vacío, y false - si no lo esta                         //
//---------------------------------------------------------------------//
...
//---------------------------------------------------------------------//
var consumer = new Kafka.KafkaConsumer({'group.id': 'kafka','metadata.broker.list': 'kafka',}, {});
consumer.connect();
consumer.on('ready', () => {
    ...
    // Pasamos la información de la cola Kafka a variables
    ...
    shell.exec(`git ls-remote "https://raulrguez09:${PERSONAL_ACCESS_TOKEN}@github.com/${repoPath}.git" >> existeRepo.txt`)
    try {
        if(fs.existsSync('./existeRepo.txt')){
            ...
            if(isEmpty === true){
                ...
                shell.exec(`git clone "https://raulrguez09:${PERSONAL_ACCESS_TOKEN}@github.com/${repoPath}.git"`)
    ...
    // Terminada la ejecución, borramos el repositorio clonado
